...
class FAQCategoryRepository extends ServiceEntityRepository
{
    ...
    private function filterWhereClause(QueryBuilder $statement, \stdClass $search)
    {
        if (isset($search->string) and Validate::not_null($search->string)) {
            $statement->andWhere('fc.id LIKE :searchTerm '
                . 'OR fc.title LIKE :searchTerm '
            );
        ...
        if (isset($search->deleted) and in_array($search->deleted, array(0, 1))) {
            if ($search->deleted == 1) {
                $statement->andWhere('fc.deleted IS NOT NULL');
            } else {
                $statement->andWhere('fc.deleted IS NULL');
            }
        }
    }
    ...
    {
        $statement->select("COUNT(DISTINCT fc.id)");
