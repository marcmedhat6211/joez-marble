...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $isRoleAdded = $this->demoteUser($email, $role);
        if ($isRoleAdded) {
            $output->writeln(sprintf('Role "%s" has been removed from user "%s". This change will not apply until the user logs out and back in again.',
                $role, $email));
        } else {
